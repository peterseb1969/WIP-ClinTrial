...
> Clinical Trials Explorer — Browse and analyze Roche/Genentech clinical trials from ClinicalTrials.gov, built on WIP (terminologies, templates, documents) with TA classification, AE term management, and an import pipeline.
...
This app's namespace is `clintrial`. It is the **production** namespace — the dev-namespace iteration phase is complete, and the app self-bootstraps into `clintrial` via its BootstrapGate (offer-on-empty / use-on-exists; see "Namespace Bootstrap on Launch" below).

**A missing namespace is never a setup failure.** The namespace is created at *bootstrap* — when there is data to put in it — not as a setup precondition. `/wip-setup` does not check for it, and an absent `clintrial` at session start is expected and non-blocking, not an error to fail over or go hunting for. (This app's `clintrial` already exists, so in practice the BootstrapGate uses it on launch; the rule matters if you ever stand the app up against a fresh WIP.)

**Why namespaces matter:** Terminologies and templates are hard to delete cleanly once documents reference them. If a future feature needs free data-model iteration, do it in a scratch namespace (e.g. `dev-clintrial-<feature>`), then recreate the finalized model in `clintrial`. Retire a scratch namespace via the API when done:
```
mcp__wip__delete_namespace(prefix="dev-clintrial-<feature>")
```
(or `DELETE /api/registry/namespaces/<prefix>`). The API honours each namespace's deletion mode (`retain` vs `full`) — no `--force` flag needed.

**Important:** MCP tool calls use the privileged admin key, so always pass `namespace=clintrial` explicitly. Your app's runtime key (scoped to one namespace) gets automatic namespace derivation — no `namespace` parameter needed in app code.
...
Because this key is scoped to a single namespace (`clintrial`), WIP derives the namespace automatically when you omit the `namespace` parameter. This means synonym resolution works without passing `namespace` on every API call.
...
- `/wip-case file|list|read|respond|comment|close|implement` — Cross-agent case management. **Cases live in the KB, not on disk** — there are no `yac-discussions/CASE-*.md` files to scan, claim, or mirror. Every read/write goes through the **served KB client** (`bash ~/.cache/wip-kb-client/kb-client.sh case-fetch.py …` to read, `kb-write.py …` to write), which talks only to the KB gateway. Each write is ONE gateway call: the gateway allocates the `case_number`, claims the `CASE-<n>` synonym atomically (race-safe by construction), and persists edges — **never hand-pick a case number, and there is no separate mirror step** (the old `case-helper.sh claim` + `add-to-kb.py` path is retired; those loaders now refuse with a pointer).
...
**Tarball location (CASE-442):** the app's pinned tarballs live in `clintrial-explorer/libs/` (gene-pool convention: versioned, immutable tarballs inside the app dir), referenced by `package.json` as `file:libs/wip-<name>-<version>.tgz` and copied into the Docker build context. The repo-root `libs/` holds only the READMEs and the wip-toolkit wheel. Current pins: client 0.21.0, react 0.13.0, proxy 0.2.0. `@tanstack/react-query` is the peer dependency that powers `@wip/react`'s hooks.
